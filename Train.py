...
def detect_syntax_errors(code_snippet: str) -> dict:
    """
    Checks the Python code for syntax errors and suggests corrections.

    Returns a dictionary with:
    - is_valid: bool
    - error_message: str (if any)
    - suggestion: str (if any)
    """
    try:
        # Attempt to parse the code to check syntax
        ast.parse(code_snippet)
        return {
            "is_valid": True,
            "error_message": None,
            "suggestion": "No issues found. Syntax appears to be correct."
        }

    except SyntaxError as e:
        # Basic syntax error caught
        error_message = f"SyntaxError on line {e.lineno}: {e.msg}"
        suggestion = ""

        # Simple suggestions based on known error types
        if "unexpected EOF while parsing" in e.msg:
            suggestion = "It looks like you're missing a closing bracket or colon."
        elif "expected an indented block" in e.msg:
            suggestion = "You may have missed indentation after a control structure (like 'if', 'for', etc)."
        elif "invalid syntax" in e.msg:
            suggestion = "Check for typos or missing symbols like colons, commas, or parentheses."

        return {
            "is_valid": False,
            "error_message": error_message,
            "suggestion": suggestion or "Please revise the code for correct syntax."
        }
